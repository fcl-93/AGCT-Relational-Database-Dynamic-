...
<?php
        if (empty($_REQUEST["data"])) {
            ...
            if (isset($_REQUEST["ateDia"])) {
                ...
                $queryHistorico = "SELECT * FROM hist_property WHERE property_id = ".$_REQUEST["id"]." AND inactive_on < '".date("Y-m-d",(strtotime($_REQUEST["data"]) + 86400))."' AND inactive_on > '".date("Y-m-d", (strtotime($_REQUEST["data"]) - 86400))."' ORDER BY inactive_on DESC LIMIT 1";
                echo $queryHistorico;
            }
        }
