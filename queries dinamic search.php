...
if(isset($_REQUEST["ent"])) {
    if ("nada selecionado") {
        ...
        $query2 = "SELECT v.value FROM property AS p, entity AS e, value AS v WHERE v.property_id = 'propriedade do tipo ent_ref' AND v.entity_id = 'id da query anterior' AND v.property_id = p.id AND e.id = v.entity_id";
        $query3 = "SELECT e.id, e.name FROM entity AS e WHERE e.id = 'id que vem pelo value da query2'";
    }
        ...
        $query2 = "SELECT e.id, e.entity_name FROM entity AS e, relation AS r WHERE (r.entity1_id = 'resultado da query anterior' OR r.entity2_id = 'resultado da query anterior')  AND (e.id = r.entity1_id OR e.id = r.entity2_id)";
    }
